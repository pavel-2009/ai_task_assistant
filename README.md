...
- NLP/RAG:
  - эмбеддинги и семантический поиск по задачам
  - RAG‑ответы через локальный Ollama
- ML/CV‑часть:
...
Для RAG-эндпоинтов нужен запущенный Ollama и загруженная модель:
...
Если Ollama работает не локально, задайте `OLLAMA_BASE_URL` и `OLLAMA_MODEL` в `.env`.
...
6. **Задать вопрос по задачам**: `POST /rag/ask`
